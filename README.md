...
- 用户在飞书群里发送 `下周三 shuyao 去杭州`，机器人创建家庭提醒，到点自动推送。
...
用户 ──飞书消息──▶ 事件接收（HTTP 回调 / WS 长连接）
               ...
               app/event_handler.py
                    ...
                    │
                    ├─▶ LLM 家务解析
                    ├─▶ 积分计算 → 写入 raw_inputs + chore_records
                    └─▶ 群内回复
```
...
| `app/reminder_service.py` | 家庭提醒业务逻辑（关键词预检、时间计算、回复格式化） |
| `app/reminder_scheduler.py` | 后台提醒调度器（每分钟扫描到期提醒并推送） |
| `app/schemas.py` | Pydantic 数据模型 |
| `app/bitable_client.py` | 多维表格客户端 |
| `app/llm_parser.py` | LLM 结构化解析（家务 + 提醒） |
...
- [x] 家庭提醒模块（reminder_records 表 + scheduler 定时推送）
...
| raw_text | 文本 | ✓ | 用户原始输入 |
| creator | 文本 | ✓ | 创建人 |
| scope | 文本 | ✓ | member（成员级）/ family（家庭级） |
| target_person | 文本 | | 具体成员名；家庭级可为空 |
| event_text | 文本 | ✓ | 事项内容 |
| event_date | 日期 | ✓ | 事项日期 |
| remind_at | 日期 | ✓ | 实际提醒时间 |
| remind_text | 文本 | ✓ | 到点推送文案 |
| chat_id | 文本 | ✓ | 飞书群 ID |
| status | 文本 | ✓ | pending / sent / failed / cancelled |
| created_at | 日期 | ✓ | 创建时间 |
| sent_at | 日期 | | 实际发送时间 |
...
FEISHU_TABLE_REMINDER_RECORDS= # reminder_records 表的 table_id
```
...
```
收到～已记录提醒：
2026-07-08 早上 08:00，我会提醒：今天 shuyao 去杭州
```
...
| 规则 | 说明 | 示例 |
|---|---|---|
| 默认提醒时间 | 用户只说日期没说几点，默认当天 08:00 | `下周三 shuyao 去杭州` → 08:00 |
| 明确时间优先 | 用户指定时间则使用指定时间 | `明天 9 点交水费` → 09:00 |
| 成员级提醒 | 提到家庭成员名则记为该成员行程 | `下周三 shuyao 去杭州` → scope=member |
| 家庭级提醒 | 未提成员名则视为家庭全体行程 | `周六全家去迪士尼` → scope=family |
| 推送位置 | 提醒到点后推回创建时的群聊 | 同群推送 |
...
- 含日期关键词但不属于提醒的内容（如 `明天我洗碗了`），LLM 判定后回退到家务流程
- 闲聊（你好、在吗）、已完成事项（我洗了碗）不会触发提醒
- 未来计划类内容（我准备拖地）不计入提醒
- 提醒到点后状态自动更新为 `sent`，不会重复发送
...
# 家庭提醒模块回归测试（mock，无需飞书/LLM）
uv run python -m pytest scripts/test_reminder_service.py scripts/test_llm_parser_reminder.py scripts/test_bitable_client_reminder.py scripts/test_event_handler_reminder_routing.py scripts/test_reminder_scheduler.py -v
...
| `FEISHU_TABLE_REMINDER_RECORDS` | 否 | reminder_records 表 table_id（未配置时提醒功能跳过写表） |
...
- [x] 家庭提醒模块（LLM 提醒解析 + reminder_records 表 + scheduler 定时推送）— Phase 6
- [x] 提醒模块回归测试（mock 覆盖 reminder_service / llm_parser / event_handler 分流 / bitable_client / scheduler）
