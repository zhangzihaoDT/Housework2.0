...
- [x] 结算失败重试支持
- [x] LLM 失败时的本地规则兜底解析（备菜→做饭、擦地→拖地等别名）
- [x] 同一句话内显式重复计分（"拖了两次地" 计 2 项）
- [x] 数据查询意图路由（近 N 天本人记录 / 本轮得分对比）
...
- [x] 结算健壮性修复（统一事件循环、错误不再误判为未结算、翻页、超时、幂等）— v0.3.0
- [x] LLM 失败本地规则兜底 + 同一句显式重复计分 — v0.3.0
- [x] 数据查询（近 N 天本人记录 / 本轮各成员得分对比）— v0.3.0
...
docker buildx build --platform linux/amd64,linux/arm64 \
  -t byte1717712/housework-feishu-bot:latest \
  -t byte1717712/housework-feishu-bot:v0.3.0 --push .
```

> 镜像地址：`docker.io/byte1717712/housework-feishu-bot`（当前版本 `v0.3.0`）。
